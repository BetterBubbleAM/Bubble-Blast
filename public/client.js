const canvas = document.getElementById('canvas');
const ctx = canvas.getContext('2d');

function resize() {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
}
window.addEventListener('resize', resize);
resize();

class Entity {
    constructor(x, y, mass, color, name, type = 'player') {
        this.x = x;
        this.y = y;
        this.mass = mass;
        this.color = color;
        this.name = name || "";
        this.type = type;
        
        // Zgodność promienia na kliencie i na serwerze (Zasada 2)
        this.radius = Math.sqrt(this.mass * 100 / Math.PI);
    }

    draw(ctx) {
        ctx.save();
        ctx.translate(this.x, this.y);
        
        if (this.type === 'virus') {
            this.drawVirus(ctx);
        } else if (this.type === 'food') {
            ctx.fillStyle = this.color;
            ctx.beginPath(); ctx.arc(0, 0, this.radius, 0, Math.PI * 2); ctx.fill();
        } else {
            this.drawCircle(ctx);
        }
        ctx.restore();
    }

    drawCircle(ctx) {
        // Kontur komórki
        ctx.strokeStyle = 'rgba(0,0,0,0.15)';
        ctx.lineWidth = 4;
        ctx.fillStyle = this.color;
        
        ctx.beginPath(); 
        ctx.arc(0, 0, this.radius, 0, Math.PI * 2); 
        ctx.fill(); 
        ctx.stroke();
        
        // Dynamiczne teksty (Zasada 2: Masa i Nick w centrum komórki)
        if (this.mass > 12 && this.type === 'player') {
            ctx.fillStyle = "white";
            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.lineWidth = 3;
            ctx.strokeStyle = 'black'; // Czarny obrys wokół tekstu by był czytelny
            
            let fontSize = Math.max(this.radius * 0.35, 10);
            ctx.font = `bold ${fontSize}px Ubuntu, sans-serif`;
            
            if (this.name) {
                ctx.strokeText(this.name, 0, -this.radius * 0.15);
                ctx.fillText(this.name, 0, -this.radius * 0.15);
            }
            
            ctx.font = `${fontSize * 0.6}px Ubuntu, sans-serif`;
            let displayMass = Math.floor(this.mass).toString();
            ctx.strokeText(displayMass, 0, this.radius * 0.25);
            ctx.fillText(displayMass, 0, this.radius * 0.25);
        }
    }

    drawVirus(ctx) {
        const spikes = 20;
        ctx.fillStyle = "#33ff33";
        ctx.strokeStyle = "#22aa22";
        ctx.lineWidth = 4;
        
        ctx.beginPath();
        for (let i = 0; i < spikes * 2; i++) {
            let r = i % 2 === 0 ? this.radius : this.radius * 0.85;
            let a = (i * Math.PI) / spikes;
            ctx.lineTo(Math.cos(a) * r, Math.sin(a) * r);
        }
        ctx.closePath(); 
        ctx.fill(); 
        ctx.stroke();
    }
}

// Socket Połączenie (Działa też na Render.com)
const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
const ws = new WebSocket(`${protocol}//${window.location.host}`);

let gameState = { players: {}, food: [], viruses:[] };
let myId = null;
let camX = 2000, camY = 2000, fovScale = 1;
let mouseX = 2000, mouseY = 2000;
let isPlaying = false;

document.getElementById('playBtn').addEventListener('click', (e) => {
    e.preventDefault();
    const nick = document.getElementById('nick').value;
    
    if (ws.readyState === WebSocket.OPEN) {
        document.getElementById('overlays').style.display = 'none';
        ws.send(JSON.stringify({ type: 'join', name: nick }));
        isPlaying = true;
    } else {
        alert("Serwer się jeszcze uruchamia, poczekaj sekundę...");
    }
});

ws.onmessage = (event) => {
    const data = JSON.parse(event.data);
    if (data.type === 'init') {
        myId = data.id;
    } else {
        gameState = data;
    }
};

window.addEventListener('mousemove', (e) => {
    if (!isPlaying) return;
    mouseX = (e.clientX - canvas.width / 2) / fovScale + camX;
    mouseY = (e.clientY - canvas.height / 2) / fovScale + camY;
    
    if (ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ type: 'move', x: mouseX, y: mouseY }));
    }
});

// Sterowanie
let macroInterval;
window.addEventListener('keydown', (e) => {
    if (!isPlaying || ws.readyState !== WebSocket.OPEN) return;
    
    if (e.code === 'Space') {
        ws.send(JSON.stringify({ type: 'split' }));
    }
    if (e.code === 'KeyW') {
        ws.send(JSON.stringify({ type: 'eject' }));
    }
    if (e.code === 'KeyE' && !macroInterval) {
        macroInterval = setInterval(() => {
            ws.send(JSON.stringify({ type: 'eject' }));
        }, 50);
    }
});
window.addEventListener('keyup', (e) => {
    if (e.code === 'KeyE') {
        clearInterval(macroInterval);
        macroInterval = null;
    }
});

function draw() {
    ctx.fillStyle = "#111111"; // Tło przypominające darkTheme z bubble
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    
    ctx.save();
    ctx.translate(canvas.width / 2, canvas.height / 2);
    ctx.scale(fovScale, fovScale);
    ctx.translate(-camX, -camY);

    // Kafelki w tle
    ctx.strokeStyle = "rgba(255, 255, 255, 0.05)";
    ctx.lineWidth = 2;
    for(let i = 0; i <= 4000; i += 50) {
        ctx.beginPath(); ctx.moveTo(i, 0); ctx.lineTo(i, 4000); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(0, i); ctx.lineTo(4000, i); ctx.stroke();
    }

    gameState.food.forEach(f => {
        new Entity(f.x, f.y, f.mass, f.color, '', 'food').draw(ctx);
    });

    gameState.viruses.forEach(v => {
        new Entity(v.x, v.y, v.mass, '#33ff33', '', 'virus').draw(ctx);
    });

    let myPlayer = gameState.players[myId];

    // Rysowanie graczy - sortowanie, by małe kulki były rysowane na wierzchu, tak jak w oryginale
    Object.values(gameState.players).forEach(p => {
        p.cells.sort((a,b) => a.mass - b.mass).forEach(c => {
            new Entity(c.x, c.y, c.mass, p.color, p.name, 'player').draw(ctx);
        });
    });

    // Płynna kamera i Dynamiczne pole widzenia (FOV) na podstawie łącznej masy
    if (myPlayer && myPlayer.cells.length > 0) {
        let cx = 0, cy = 0, totalMass = 0;
        myPlayer.cells.forEach(c => { 
            cx += c.x; cy += c.y; totalMass += c.mass; 
        });
        cx /= myPlayer.cells.length;
        cy /= myPlayer.cells.length;
        
        camX += (cx - camX) * 0.1;
        camY += (cy - camY) * 0.1;
        
        let targetFov = Math.max(0.1, 1.5 - Math.log10(totalMass) * 0.25);
        fovScale += (targetFov - fovScale) * 0.1;
    }

    ctx.restore();
    requestAnimationFrame(draw);
}
draw();
